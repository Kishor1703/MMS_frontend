...
const COMPONENTS = [
  { key: "compressorOil", label: "Compressor Oil", type: "basic" },
  { key: "oilFilter", label: "Oil Filter", type: "basic" },
  { key: "airFilter", label: "Air Filter", type: "basic" },
  { key: "oilSeparator", label: "Oil Separator", type: "oilSeparator" },
  { key: "separatorElement", label: "Separator Element", type: "basic" },
  { key: "coolant", label: "Coolant", type: "coolant" },
];
...
const emptyFactory = {
  basic: emptyBasic,
  oilSeparator: emptyOilSeparator,
  coolant: emptyCoolant,
};
...
function OilSeparatorInspector({ data, update }) {
  return (
    <div className="insp-group">
      <div className="insp-grid">
        <SelectRow label="Separator Condition" value={data.separatorCondition} options={BASIC_CONDITIONS} onChange={(v) => update("separatorCondition", v)} />
        <SelectRow label="Differential Pressure" value={data.differentialPressure} options={["Normal", "High", "Critical"]} onChange={(v) => update("differentialPressure", v)} />
        <SelectRow label="Oil Carryover" value={data.oilCarryover} options={["None", "Slight", "Heavy"]} onChange={(v) => update("oilCarryover", v)} />
        <SelectRow label="Separator Element Condition" value={data.separatorElementCondition} options={["Good", "Worn", "Clogged", "Replace"]} onChange={(v) => update("separatorElementCondition", v)} />
        <SelectRow label="O-Ring / Seal Condition" value={data.oringSealCondition} options={["Good", "Cracked", "Hardened", "Leaking"]} onChange={(v) => update("oringSealCondition", v)} />
        <SelectRow label="Cleaning Status" value={data.cleaningStatus} options={["Cleaned", "Not Cleaned", "Replace"]} onChange={(v) => update("cleaningStatus", v)} />
        <SelectRow label="Replacement Status" value={data.replacementStatus} options={["Not Required", "Required", "Replaced"]} onChange={(v) => update("replacementStatus", v)} />
        <DateRow label="Replacement Date" value={data.replacementDate} onChange={(v) => update("replacementDate", v)} />
        <DateRow label="Next Replacement Date" value={data.nextReplacementDate} onChange={(v) => update("nextReplacementDate", v)} />
      </div>
      <label className="insp-field">
...
function CoolantInspector({ data, update }) {
  return (
    <div className="insp-group">
      <div className="insp-grid">
        <SelectRow label="Coolant Level" value={data.coolantLevel} options={["Full", "Low", "Empty"]} onChange={(v) => update("coolantLevel", v)} />
        <SelectRow label="Coolant Condition" value={data.coolantCondition} options={["Clear", "Discoloured", "Contaminated"]} onChange={(v) => update("coolantCondition", v)} />
        <SelectRow label="Coolant Leakage" value={data.coolantLeakage} options={["None", "Minor", "Major"]} onChange={(v) => update("coolantLeakage", v)} />
        <SelectRow label="Cooler / Radiator Condition" value={data.coolerRadiatorCondition} options={["Good", "Clogged", "Leaking", "Replace"]} onChange={(v) => update("coolerRadiatorCondition", v)} />
        <DateRow label="Coolant Replacement Date" value={data.coolantReplacementDate} onChange={(v) => update("coolantReplacementDate", v)} />
        <DateRow label="Next Coolant Service Date" value={data.nextCoolantServiceDate} onChange={(v) => update("nextCoolantServiceDate", v)} />
      </div>
      <label className="insp-field">
        <span className="insp-label">Notes</span>
        <textarea rows={2} value={data.notes || ""} onChange={(e) => update("notes", e.target.value)} />
...
const INSPECTORS = {
  ...
  oilSeparator: OilSeparatorInspector,
  coolant: CoolantInspector,
};

/* ───────────────────────── Main tab ───────────────────────── */
export default function CompressorMaintenanceTab({ machineId, userRole }) {
  ...
  const load = useCallback(() => {
    Promise.all([
      compressorMaintenanceApi.list({ machine: machineId, limit: 200 }),
      compressorMaintenanceApi.schedule(machineId),
    ])
      .then(([recordsRes, scheduleRes]) => {
        setRecords(recordsRes.data.data);
        setSchedule(scheduleRes.data.data);
      })
      .catch((err) => setError(err.response?.data?.message || "Failed to load compressor maintenance"))
    ...
    try {
      ...
      const components = Object.fromEntries(
        COMPONENTS.map((c) => {
          ...
          if (c.key === "oilSeparator") {
            comp.replacementDate = sanitizeDate(comp.replacementDate);
            comp.nextReplacementDate = sanitizeDate(comp.nextReplacementDate);
          }
          if (c.key === "coolant") {
            comp.coolantReplacementDate = sanitizeDate(comp.coolantReplacementDate);
            comp.nextCoolantServiceDate = sanitizeDate(comp.nextCoolantServiceDate);
          }
          return [c.key, comp];
        })
      );
      ...
      await compressorMaintenanceApi.create(payload);
      setShowForm(false);
      load();
    } catch (err) {
      setError(err.response?.data?.message || "Failed to save compressor maintenance");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="compressor-tab">
      <div className="sp-header">
        <div>
          <h2 className="sp-title">Compressor Maintenance</h2>
          <p className="sp-subtitle">Inspect compressor oil, filters, separator and coolant; record spare parts and costs.</p>
        </div>
        {canCreate && <button className="btn-primary" onClick={openForm}>+ Log Compressor Maintenance</button>}
      </div>
      ...
      {!loading && records.length === 0 && (
        <div className="sp-empty">
          <p>No compressor maintenance logged yet.</p>
              ...
              {isOpen && (
                <div className="job-detail">
                  <div className="job-timeline">
                    ...
                    <TimelineRow num="2" label="Components Checked" value={recordSummary(record)} />
                    <TimelineRow num="3" label="Final Status" value={record.finalStatus || "—"} />
                    <TimelineRow num="4" label="Remarks" value={record.remarks || "—"} />
                    <TimelineRow num="5" label="Next Maintenance" value={record.nextMaintenanceDate ? new Date(record.nextMaintenanceDate).toLocaleDateString() : "—"} />
                    <TimelineRow num="6" label="Cost" value={`$${totalCost.toFixed(2)}`} highlight="#4f46e5" />
                  </div>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal-box cmp-modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="mj-modal-title">Log Compressor Maintenance</h3>
